...
class Nav extends Component {
  ...
  handleScroll = () => {
    const isSolid = window.pageYOffset != 0;
  ...
  curtainAnimation = visibility => {
    const tl = TweenLite.to(this.curtain, config.time.default, {
  ...
  render() {
    const {
      dispatch,
      menu: { isVisible, menuHeight, action },
      warning,
      setWarningHeight,
      resolveWarning,
      toggleMenuVisibility,
    ...
    return (
      <div>
        {warning.message ? (
          <Warning
            {...warning}
            setHeight={setWarningHeight}
            resolve={resolveWarning}
          />
        ) : null}
        <Wrapper
          id="app-nav"
          innerRef={node => (this.nav = node)}
          isVisible={isVisible}
          solid={solid}
