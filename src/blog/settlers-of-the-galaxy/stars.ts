...
export type StarType = Float32Array;
...
export function getAngularVelocity(radius: number): number {
...
export function getPositionsAtTime(time: number, star: StarType) {
...
export default async function* asyncStarsIterator(
  ...
  for await (const chunk of asyncStreamIterator(response.body)) {
    ...
    const numStars = (combinedChunk.byteLength - leftover) / BYTES_PER_STAR;
